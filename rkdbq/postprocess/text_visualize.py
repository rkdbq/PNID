...
# ann_dir_path = "D:\\Experiments\\Detections\\old\\roi_trans_old\\test\\annfiles_123"
# write_path = f"D:\\Experiments\\Text_Merge\\{datetime_string}"

# merge = text_merge(ann_dir_path).write(write_path)

# Path(vis_dir).mkdir(parents=True, exist_ok=True)
# # Path(diagram_dir).mkdir(parents=True, exist_ok=True)
# # convert_class_to_diagram(f"{write_path}\\", diagram_dir, 0.8)

# gt_result = diagram_text_to_dic(gt_dir)
# dt_result = diagram_text_to_dic(f"{write_path}\\")
